...
cv2.namedWindow('Filtro Promediado',cv2.WINDOW_AUTOSIZE)
cv2.createTrackbar('Tipo','Filtro Promediado',0,10,nothing)
while True:
        ...
        N = cv2.getTrackbarPos('Tipo', 'Filtro Promediado')
        if N!=0:
            
            N= 2*N + 1
            imagen = cv2.blur(imagen, (N,N))
        ...
        #Mostramos imagen  
        cv2.imshow('Filtro Promediado', imagen)
